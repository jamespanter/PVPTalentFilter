...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
	if event == "ADDON_LOADED" and arg1 == "PVPTalentFilter" then
		-- Set hidden talents saved variable if none
		if not PTF_HiddenTalents then
...
function isTalentHidden(talentID)
	for _, id in ipairs(PTF_HiddenTalents) do
		if id == talentID then
			return true
		end
	end
...
function addTalentToHiddenList(talentID)
	if not isTalentHidden(talentID) then
		table.insert(PTF_HiddenTalents, talentID)
		print("|cffffcc00PVPTalentFilter - TalentID " .. talentID .. " |cffff0000hidden|r.|r")
	else
...
function removeTalentFromHiddenList(talentID)
	for i, id in ipairs(PTF_HiddenTalents) do
		if id == talentID then
			table.remove(PTF_HiddenTalents, i)
